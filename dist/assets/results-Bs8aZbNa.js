import{o as le,l as w,c as Z,k as de,m as ie,n as ce,d as J,t as b,e as f,j as ue}from"./admin-MwW8t9Ay.js";import{query as K,collection as _,where as ee,orderBy as te,getDocs as ne}from"https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js";import{g as me}from"./services-B8oy3dvD.js";import{s as pe}from"./resources-BwAOLOsg.js";let u=[],d=[],y=1;const Q=10;let S=null,R=null;const T={auto:"AT-AVE-AVEX",loisir:"VTT, Motoneige, etc.",vr:"Véhicules Récréatifs",tracteur:"Équipement Agricole"},ge={auto:"#C41E3A",loisir:"#D4AF37",vr:"#FF9F43",tracteur:"#28A745"};document.addEventListener("DOMContentLoaded",async()=>{le(async t=>{if(!t){window.location.href="/index.html";return}fe(t);const n=await me(t.uid);n&&n.role==="admin"&&(document.getElementById("nav-admin-item")?.classList.remove("hidden"),document.getElementById("admin-badge-nav")?.classList.remove("hidden")),await he(t.uid)}),document.getElementById("logout-btn")?.addEventListener("click",se),document.getElementById("signout-link")?.addEventListener("click",se),document.getElementById("filter-module")?.addEventListener("change",D),document.getElementById("filter-period")?.addEventListener("change",D),document.getElementById("filter-sort")?.addEventListener("change",D),document.getElementById("reset-filters-btn")?.addEventListener("click",Be),document.getElementById("prev-page-btn")?.addEventListener("click",()=>M(-1)),document.getElementById("next-page-btn")?.addEventListener("click",()=>M(1)),document.getElementById("export-results-btn")?.addEventListener("click",ke);const e=document.getElementById("results-list");e&&!e.dataset.eventsBound&&(e.addEventListener("click",re),e.dataset.eventsBound="true")});function fe(e){document.getElementById("user-name")&&(document.getElementById("user-name").textContent=e.displayName||e.email),document.getElementById("user-photo")&&(document.getElementById("user-photo").src=e.photoURL||`https://ui-avatars.com/api/?name=${encodeURIComponent(e.displayName||e.email)}`),document.getElementById("user-display-name")&&(document.getElementById("user-display-name").textContent=e.displayName||e.email),document.getElementById("user-avatar")&&(document.getElementById("user-avatar").src=e.photoURL||`https://ui-avatars.com/api/?name=${encodeURIComponent(e.displayName||e.email)}`)}async function he(e){w.info("📥 Chargement des résultats pour:",e);try{const t=document.getElementById("global-stats"),n=document.getElementById("results-list"),s=document.getElementById("progress-chart-container");t&&Z("global-stats",de(4)),n&&Z("results-list",ie(5)),s&&(s.innerHTML=ce());let o=K(_(J,"quizResults"),ee("userId","==",e),te("completedAt","desc")),a=await ne(o);a.empty&&(o=K(_(J,"quizResults"),ee("userId","==",e),te("date","desc")),a=await ne(o)),u=[],a.forEach(r=>{const i=r.data();let c=null;if(i.completedAt&&typeof i.completedAt.toDate=="function")c=i.completedAt.toDate();else if(i.date&&typeof i.date.toDate=="function")c=i.date.toDate();else if(i.date&&typeof i.date=="string"){const l=new Date(i.date);isNaN(l.getTime())||(c=l)}u.push({id:r.id,...i,completedAt:c})}),w.info(`✅ ${u.length} résultats chargés`),u.length===0?H():(d=[...u],oe())}catch(t){w.error("❌ Erreur lors du chargement des résultats:",t),t.code==="permission-denied"?b.error("Permissions insuffisantes pour consulter vos résultats. Contactez un administrateur."):b.error("Erreur lors du chargement des résultats"),H()}}function xe(){const e=document.getElementById("results-container");if(!e||document.getElementById("results-list"))return;e.innerHTML=`
    ...
    `;const t=document.getElementById("prev-page-btn"),n=document.getElementById("next-page-btn");t&&!t.dataset.eventsBound&&(t.addEventListener("click",()=>M(-1)),t.dataset.eventsBound="true"),n&&!n.dataset.eventsBound&&(n.addEventListener("click",()=>M(1)),n.dataset.eventsBound="true");const s=document.getElementById("results-list");s&&!s.dataset.eventsBound&&(s.addEventListener("click",re),s.dataset.eventsBound="true")}function oe(){if(d.length===0){H();return}document.getElementById("no-results")?.classList.add("hidden"),xe(),ve(),be(),ae(),document.getElementById("results-loading")?.classList.add("hidden"),document.getElementById("results-container")?.classList.remove("hidden")}function ve(){const e=document.getElementById("global-stats"),t=d.length,n=t>0?Math.round(d.reduce((c,l)=>c+l.score,0)/t):0,s=d.reduce((c,l)=>c+l.totalQuestions,0),o=d.reduce((c,l)=>c+l.correctAnswers,0),a=d.reduce((c,l)=>c+(l.timeSpent||0),0),r=t>0?Math.round(a/t):0,i=f(U(r));e.innerHTML=`
    ...
    `}function be(){ye(),Ee()}function ye(){console.log("🎯 updateProgressChart() appelée");const e=document.getElementById("progress-chart");if(console.log("📊 Canvas ctx:",e),console.log("📊 Chart.js disponible?",typeof Chart<"u"),!e){console.error("❌ Canvas #progress-chart introuvable!");return}const t=[...d].reverse().slice(-10);if(console.log("📊 recentResults:",t),t.length===0){console.warn("⚠️ Aucune donnée pour le graphique de progression"),console.log("📊 allResults.length:",u.length),console.log("📊 filteredResults.length:",d.length);const o=e.closest(".bg-white");o&&(o.innerHTML=`
            ...
            `);return}const n=t.map((o,a)=>{const r=o.completedAt;return r?`${r.getDate()}/${r.getMonth()+1}`:`Quiz ${a+1}`}),s=t.map(o=>o.score);console.log("📊 labels:",n),console.log("📊 scores:",s),S&&S.destroy(),S=new Chart(e,{type:"line",data:{labels:n,datasets:[{label:"Score (%)",data:s,borderColor:"#C41E3A",backgroundColor:"rgba(196, 30, 58, 0.1)",tension:.4,fill:!0,pointRadius:5,pointHoverRadius:7,pointBackgroundColor:"#C41E3A",pointBorderColor:"#fff",pointBorderWidth:2}]},options:{responsive:!0,maintainAspectRatio:!0,aspectRatio:1.5,plugins:{legend:{display:!1}},scales:{y:{beginAtZero:!0,max:100,ticks:{callback:o=>o+"%"}}}}}),console.log("✅ Chart progression créé:",S)}function Ee(){console.log("🎯 updateModuleChart() appelée");const e=document.getElementById("module-chart");if(console.log("📊 Canvas module ctx:",e),!e){console.error("❌ Canvas #module-chart introuvable!");return}const t={};if(d.forEach(a=>{const r=a.moduleId||a.module||"auto";t[r]=(t[r]||0)+1}),console.log("📊 moduleCounts:",t),Object.keys(t).length===0){console.warn("⚠️ Aucune donnée pour le graphique de modules"),console.log("📊 allResults.length:",u.length),console.log("📊 filteredResults.length:",d.length);const a=e.closest(".bg-white");a&&(a.innerHTML=`
                <div class="p-8 flex flex-col items-center justify-center" style="min-height: 350px;">
